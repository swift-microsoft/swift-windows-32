...
// ===----------------------------------------------------------------------===//
    ...
    extension Windows.`32`.Kernel.IO.Completion.Port {
        ...
        /// ```swift
        /// // Single completion
        /// let item = try Windows.`32`.Kernel.IO.Completion.Port.Dequeue.single(port, timeout: INFINITE)
        /// switch item.status {
        /// case .ok:
        ...
        ///     try Windows.`32`.Kernel.IO.Completion.Port.Dequeue.batch(port, entries: buffer, timeout: 100)
        /// }
        /// ```
        ///
        /// ## See Also
    ...
    extension Windows.`32`.Kernel.IO.Completion.Port.Dequeue {
        ...
        /// convenience (`single(_:timeout:)` taking `Windows.`32`.Kernel.Descriptor`)
        ...
        ///   - timeout: Timeout in milliseconds (`Windows.`32`.Kernel.IO.Completion.Port.Error.Timeout.infinite` = 0xFFFFFFFF).
        ...
        /// convenience (`batch(_:entries:timeout:)` taking `Windows.`32`.Kernel.Descriptor`)
        /// delegates to this raw SPI internally via `descriptor._rawValue`.
        ///
